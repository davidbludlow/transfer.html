...
4. **Shor-immunity on the wire.** Nothing that breaks under Shor's algorithm protects data in transit. The strict requirement is Shor-immunity, not "no asymmetric crypto" — post-quantum asymmetric primitives (e.g., ML-KEM / Kyber, ML-DSA / Dilithium) are also Shor-immune and would be acceptable in principle. The current design satisfies Shor-immunity trivially: a 256-bit pre-shared key plus AES-256-GCM, with no key exchange of any kind. AES gives 128-bit post-quantum symmetric strength under Grover's algorithm, which is unbreakable. The current design avoids asymmetric primitives by choice (see "Cryptographic preferences" below) for audit-surface reasons, not Shor reasons.
5. **Auditability.** The whole thing should remain small. Originally aspired to HTML under 400 lines and relay under 100 lines; current reality is ~700 HTML / ~225 relay, exceeded for justified reasons (browser-cap workarounds for >2 GiB transfers; proper relay backpressure; optional metering-sidecar wiring; a plain-language how-it-works section and online-version banner in the HTML; the relay also serving the page). Resist the urge to add frameworks, build steps, or further dependency trees beyond the one current `npm:ws` import. Within `transfer.html`, prefer HTML and CSS over JavaScript when either can do the job, because declarative markup and stylesheets are dramatically easier to scan for malicious behavior than imperative code, and almost every interesting attack lives in JS.

## The hosted version

The relay also serves `transfer.html` at `/` (read-only, one file), so there's a hosted copy at `transfer-html.fly.dev` for people who can't run a local file. That copy loads over the network, so it does **not** get Goal #1 — each visit trusts the host to serve honest code. Frame it honestly as the convenience tier: the online-mode banner and the README steer users to download + verify, and any reference to the online URL inside the page should carry the same trust caveat.

A gitignored `redirect-app/` deploys `transferhtml.fly.dev`, which 301-redirects the no-hyphen look-alike here.
...
- **No `--allow-all` for the relay.** The relay needs `--allow-net`, a narrowly-scoped `--allow-env` for `ws`'s three optional config reads (`WS_NO_BUFFER_UTIL`, `WS_NO_UTF_8_VALIDATE`, `NODE_ENV`) and the two optional sidecar config reads (`SIDECAR_URL`, `SIDECAR_TOKEN`), and a single-file `--allow-read=transfer.html` so it can serve the page at `/` (read once at startup; without the permission or the file, page-serving is silently disabled and it runs as a pure relay). The only disk access is that one read-only file; no subprocess, no broader env access. If the sidecar is enabled, `--allow-net` also needs the sidecar's host added (e.g. `your-sidecar.workers.dev:443`).
...
- **Never weaken the cipher.** AES-256-GCM is the minimum. Don't downgrade to AES-128 even if "the shared key only has 128 bits of entropy" — Grover halves it.
- **Never store keys.** No password managers, no autofill, no cached form data. The HTML actively suppresses browser persistence on input fields. Preserve those attributes (`autocomplete=off`, `autocorrect=off`, `autocapitalize=off`, `spellcheck=false`, `data-form-type=other`, no `name=` attribute, no enclosing `<form>`).
...
- **No key exchange at all.** The pre-shared-key + AEAD design is the simplest way to satisfy Shor-immunity, and it minimizes the cryptographic audit surface to HKDF + AES-GCM. If you must add a key exchange — for example to gain forward secrecy — it must be post-quantum (ML-KEM or a hybrid like X25519+ML-KEM where the X25519 half is treated as defense-in-depth, not load-bearing). Any key-exchange addition is a non-trivial expansion of trust surface; argue for it in writing before merging.
...
- From a single shared key, HKDF-SHA-256 derives the 128-bit room ID and the 256-bit AES key. Distinct `info` strings ("transfer-room-v1", "transfer-key-v1") provide domain separation.
- The shared key is 256 random bits, base64url-encoded. This matches the AES-256 key size so Grover gives the same 128-bit post-quantum strength as the cipher; a shorter shared key would be the weaker link. If you change `KEY_BYTES`, do not go below 32 — and do not change the encoding.
...
- Anything that requires the relay to do more than forward bytes or serve the one static page (see "The hosted version") — no per-request logic, no request-dependent responses, no second served file.
