...
5. **Auditability.** The whole thing should remain small. Roughly: HTML under 400 lines, relay under 100 lines. Resist the urge to add frameworks, build steps, or dependency trees.
...
`test-relay.ts` exercises the relay's WebSocket forwarding. The HTML's crypto path is best validated by manual round-trip testing in a browser. If you add an automated end-to-end test, either:

- Drive it via a headless browser, or
- Replicate the HTML's crypto helpers in Deno and run a TypeScript-only round trip.
