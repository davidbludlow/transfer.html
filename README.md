...
For LAN-only use, run it on any machine on your network. The HTML connects via `ws://host:8080/`. For internet use, put it behind a reverse proxy that terminates TLS, and use `wss://`.
...
The shared secret is 128 random bits, base64url-encoded (~22 characters). Generated by the sender, transmitted to the receiver out-of-band (chat message, in person, etc.).
